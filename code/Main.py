...
pytesseract.pytesseract.tesseract_cmd = r'D:\Mini Project\tesseract.exe'  # Update this path

# Load the dataset
df = pd.read_csv(r'D:\Mini Project\code\cleaned_medicine_dataset_cleaned.csv')  # Update this path

# Common terms to ignore for matching
...
# Function to query OpenAI's API for additional information or responses
...
# Main function with conversational input and handling
def main():
    ...
    # Step 3: Display the result if a close match is found
    if similarity_score > 0.5:  # Adjust threshold as needed
        ...
        if result:
            ...
            print(result)
            handle_conversation(extracted_text)  # Start conversation even if found online
